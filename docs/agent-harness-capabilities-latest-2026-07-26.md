...
| C02 模型、Provider 与能力绑定 | 🟡 部分实现 | BYOK 密钥在系统凭据库；Run 冻结具体 `local:<connection>:<model>`；模型资料约束上下文、输出、工具与图片能力；禁止静默 Provider/模型回退。 | 官方声明与静态 metadata 仍不能证明当前 key、地域端点、adapter 和流式工具调用真实可用；真实“模型→工具→模型”离线 Gate 仍是路线图第一项。 | [`model_services/`](../runtime/src/shejane_runtime/model_services/)、[`profiles.py`](../runtime/src/shejane_runtime/model_services/profiles.py)、[`llm/`](../runtime/src/shejane_runtime/llm/)、[`roadmap.md`](./roadmap.md) |
