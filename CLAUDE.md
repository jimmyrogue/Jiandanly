...
| Runtime code | `runtime/src/shejane_runtime/`: `server.py` exposes the local API; `runs.py` owns execution, cleanup, and settlement; `agent/builder.py` assembles reusable Agent definitions; `agent/subagents.py` defines Deep Agents subagents; `middleware/` owns input observation, output policy, tool visibility, human approval, tool receipts, and the single completion route; `tools/` contains tool implementations; `store/sqlite.py` is the stable persistence facade and `store/` owns domain state and durable job leases (see `docs/runtime-store.md`) |
