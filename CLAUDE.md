...
| Runtime model services | `runtime/src/shejane_runtime/model_services/`: `routes.py` owns connection lifecycle, `catalog_routes.py` owns catalog entries and capability bindings, `authorization_routes.py` owns official authorization, and the remaining modules own credentials, profiles, probes, and catalog projection; `runtime/src/shejane_runtime/llm/`, `client/src/features/settings/ModelServicesSettings.tsx` |
| Runtime code | `runtime/src/shejane_runtime/`: `server.py` assembles the local HTTP interface; `command_routes/` keeps the single `/v1/commands` contract while separating plugin control commands from Run wait decisions; `runs/` owns Run admission, routes, lifecycle, P5 execution, P7 graph driving, projections, interrupts, settlement, and event publication behind the stable `shejane_runtime.runs` interface; root `run_configuration.py` is the cycle-free Agent/Run policy seam; `agent/` assembles Agent definitions and per-attempt tools; `middleware/` owns model/tool policies; `plugins/registry/` owns plugin control-plane implementation; `tools/{image,mcp,office}/` own tool-family implementations; `store/sqlite.py` is the stable persistence facade and `store/` groups state owners by domain (see `docs/runtime-store.md`) |
| Client code | `client/src/`: `App.tsx` is the composition root; `features/app/runtime/` owns P2 command delivery, decision-command settlement, and Runtime observers; `features/chat/projection/` owns P4 snapshot/SSE projection; `features/chat/components/{editor,progress}/` group the editor and run-progress implementations; `runtime/client.ts` adapts `@shejane/runtime-sdk` to Electron |
...
- SSE parsing and the `AgentRunEvent` union live in `runtime/sdk`. New Runtime events also need a Client projection in `features/chat/projection/chatStore.ts` and/or `App.tsx`.
