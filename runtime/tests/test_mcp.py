...
`monkeypatch.setenv("SHEJANE_RUNTIME_MCP_DISCOVERY", "on")` and point
...
from shejane_runtime.tools.mcp import (
...
async def test_supervisor_times_out_a_stuck_tool_and_retires_the_session(
    ...
    import shejane_runtime.tools.mcp as mcp_mod
...
async def test_runtime_catalog_reuses_tools_until_server_config_changes(
    ...
    monkeypatch.setenv("SHEJANE_RUNTIME_MCP_SERVERS", "{}")
    await catalog.get_tools()
    monkeypatch.setenv(
        "SHEJANE_RUNTIME_MCP_SERVERS",
        json.dumps({"docs": {"command": "node", "args": ["server-v2.js"]}}),
...
async def test_runtime_catalog_hydrates_without_connecting_server(
    ...
    monkeypatch.setenv("SHEJANE_RUNTIME_MCP_SERVERS", json.dumps({"docs": config}))
    store = await LocalStore.open(tmp_path / "store.db")
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    """Spin up the runtime with an isolated tmp data dir + tok auth.

    Doesn't enable disk scan — tests that want disk scan flip the
    SHEJANE_RUNTIME_MCP_DISCOVERY env per test body.
    """
...
def test_http_mcp_servers_empty_default(client: TestClient) -> None:
    ...
    # drop configs.
    r = client.get("/v1/mcp-servers", headers={"Authorization": "Bearer tok"})
...
def test_http_mcp_server_crud_writes_only_shejane_config(
    ...
    listed = client.get("/v1/mcp-servers", headers={"Authorization": "Bearer tok"})
